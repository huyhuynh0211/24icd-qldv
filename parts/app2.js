...
TaiLieu:{
  ...
  upload(){
    ...
    const saveMeta=(driveUrl)=>{
      ...
      DB.set('tailieu',tl);
      const tb=DB.get('thongbao')||[];tb.unshift({id:'TB'+Date.now(),tieuDe:'Tài liệu mới',noiDung:'Tài liệu "'+title+'" đã được upload.',loai:'info',daDoc:false,thoiGian:new Date().toISOString()});DB.set('thongbao',tb);App.ThongBao.render();
